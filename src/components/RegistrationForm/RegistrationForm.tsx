...
const RegistrationForm = () => {
    ...
    useEffect(() => {
        const setRegistrationState = async () => {
            try {
                ...
                const registration = forms.filter((state) => state.form === "registration");
    ...
    const onSubmit = async (data: registrationType) => {
        try {
            const newDocRef = doc(registrationCollectionRef);
            await setDoc(newDocRef, data);
            const updatedDocRef = doc(db, "registration", "zzz" + Date.now().toString());
            await setDoc(updatedDocRef, data);
        } catch (e) {
    ...
    if (isFetchError) {
        ...
        return (
            ...
            >
                <Typography variant='h4' className=' text-center pb-4 text-primary-color'>
                    {languageMode == "english" ? "Registration is closed" : "Zapisy są zamknięte"}
                </Typography>
                <Typography variant='h5' className='text-primary-color p-4'>
                    {languageMode == "english" ? "Try again later" : "Spróbuj ponownie później"}
                </Typography>
            </AnimateWrapper>
        );
    }
    ...
    return (
        <FormProvider {...methods}>
            <form onSubmit={methods.handleSubmit(onSubmit)}>
                <AnimateWrapper duration={1} delay={0.1} type='FadeInBottom' once={true}>
                    <Typography variant='h3' className=' text-center pb-4'>
                        {languageMode == "english" ? "Sign up for the trip" : "Zapisz się na wyjazd"}
                    </Typography>
                </AnimateWrapper>
                <AnimateWrapper duration={1} delay={0.2} type='FadeInBottom' once={true}>
                    <ParticipantInfoSection />
                </AnimateWrapper>
                <AnimateWrapper duration={1} delay={0.3} type='FadeInBottom' once={true}>
                    <StudentDetailsSection />
                </AnimateWrapper>
                <AnimateWrapper duration={1} delay={0.4} type='FadeInBottom' once={true}>
